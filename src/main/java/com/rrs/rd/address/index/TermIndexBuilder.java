...
public class TermIndexBuilder {
	private final TermIndexEntry indexRoot = new TermIndexEntry();
	...
	public TermIndexBuilder(AddressPersister persister, List<String> ingoringRegionNames){
		...
		this.indexIgnoring(ingoringRegionNames);
	}
	
	/**
	 * 为行政区划建立倒排索引。
	 */
	public void indexRegions(List<RegionEntity> regions){
	 ...
	 * 为忽略列表建立倒排索引
	 */
	public void indexIgnoring(List<String> ignoreList){
		if(ignoreList==null || ignoreList.isEmpty()) return;
